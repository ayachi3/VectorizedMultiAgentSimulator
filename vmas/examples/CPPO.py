...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        mean = torch.stack(torch.chunk(mean, chunks=4, dim=1), dim=0)
        variance = torch.stack(torch.chunk(variance, chunks=4, dim=1), dim=0)
...
class Agent:
    def __init__(self, state_dim, action_dim, config, agent_num, env):
        ...
        self.action_dim = action_dim
        # 因为是集中化的，因此需要将单个智能的观察汇总，也就出现了乘法
        self.actor = Actor(state_dim * agent_num, action_dim * agent_num, config=config)
        self.critic = Critic(state_dim * agent_num, config)
        # NOTE: 未来可以添加学习率的动态更新

        self.memory = Memory(config.mini_batch_size, state_dim * agent_num, action_dim * agent_num)
        # 开始时不容易找到好的结果，就随机探索
        self.epsilon = 1
    ...
    def get_action(self, state):
        ...
        action = torch.rand(4, self.config.env_num, self.action_dim).to(self.device)
        action = action * 2 - 1
        action = action * torch.sqrt(variance) + mean
        # 标准化
    ...
    def test(self, step_limit, iter: int):
        ...
        while step_limit > 0 and not done.all():
            ...
            # 输入的action的规模应该是 4 * env_num * action_dim
            '''
            env.step() pass action as parameter, this is a list-like object, and use tensor would be fine.
            It will return a tuple, which contain information of all environments after such step
            action shape is (agent number) * (environment number) * (action dimension)
            
            state is each agent's observation, 
            and its size is (agent number) * (environment number) * (state dimension)
            
            reward is whether the step gets reward or not,
            and its size is (agent number) * (environment number)
            
            done is boolean value which describe whether this round comes to an end
            and its size is (agent number)
            
            info contains other information about environment, but it is not used in the project
    ...
    def learn(self):
        ...
        for i in range(self.epoch_num):
            ...
            # 差值用评论家计算当前的状态价值
            delta = td_target - self.critic(s).squeeze()
            # 计算上一状态选到特定数值的概率
            mean, variance = self.actor.forward(s)  # 用均值表示概率

            mean = torch.stack(torch.chunk(mean, dim=1, chunks=self.config.mini_batch_size), dim=0)  # 20*4*13*2  13 is env_num
...
def get_args():
    ...
    parser.add_argument('--env_num', default=256, type=int, help='number of environments')
...
def run_CPPO(
    ...
    # make agent
    agent = Agent(n_states, n_actions, cfg, len(env.observation_space), env)
    agent.learn()
