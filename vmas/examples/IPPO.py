...
class Agent:
    ...
    def act_with_env(self, step_limit: int):
        ...
        while step_limit > 0 and not done.all():
            ...
            '''此处不能将智能体的奖励相加，因此可以更好的了解每个智能体行动的好坏'''
            done = done.detach()
            ''' done 必须取反 '''
            done = ~done

            # 使用原本的API
            '''此时state不需要进行状态的压缩了，不是增加一个维度,因为到后面还要拆开'''
            '''reward 与 done 都是要给4个共用的'''
            self.memory.push(state=torch.cat(state, dim=0).to(self.device).detach(),
                             action=action,
                             reward=torch.cat(reward,dim=0).to(self.device),
                             next_state=torch.cat(next_state, dim=0).to(self.device),
                             done=torch.cat([done]*self.config.agent_num,dim=0).to(self.device))
    ...
    def learn(self):
        ...
        for i in range(self.epoch_num):
            ...
            ''' mean variance : (batch_size*agent_num*env_num) x action_dim'''
            ''' 保证均不transpose即可'''
            ...
            loss = -torch.log(prob) * delta.reshape((delta.shape[0], 1)) + \
                   F.smooth_l1_loss(self.critic(s).squeeze(), td_target.squeeze())
...
def get_args():
    ...
    parser.add_argument('--algo_name', default='IPPO', type=str, help="name of algorithm")
    parser.add_argument('--env_num', default=11, type=int, help='number of environments')
